...
//
//  coalescing(block, list_of_blocks):
//    for (b in list_of_blocks):
//      keep track of ALL block's pointers (prev[|k|], k = block in heap sequence)
//    endfor
//
//      if (*block + size + 1 = b):
//        merge block and b, edit next and prev[k] values, set id.
//    endif
//    if (*block - prev->size - 1 = prev):
//      merge block and prev, edit next and prev[k] values, set id.
//    endif
//
//    (The above if statements aren't exclusive, so the case with before and after are good.
//  return;
